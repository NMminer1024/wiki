...
- The pool diff is set too high. Switch to NMMiner's official [SoloBTC](https://solobtc.nmminer.com) pool (or the official fallback pool).
- Verify the wallet field — many pools reject incorrectly formatted wallet addresses. See [FAQ](./faq) for valid Bitcoin address formats.
