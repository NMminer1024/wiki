...
End-to-End-Snippets, die Sie in ein Dashboard, einen Cron-Job oder einen Discord-Bot einbinden können.

---

## cURL — Live-Hashrate auslesen
...
Gibt die aktuelle Hashrate in **H/s** aus.

---

## cURL — Neue Pool-URL an einen Schwarm von Minern senden
...
## Python — Gesamte LAN-Hashrate

```python
import requests

# Wählen Sie einen beliebigen erreichbaren Miner als Seed für die Discovery.
seed = "192.168.1.42"
...
print(f"Gesamte LAN-Hashrate: {total_hs/1e3:,.1f} KH/s über {len(ips)} Miner")
```

---

## Python — Jeden Miner physisch finden
...
Gehen Sie durch das Haus — jeder Miner blinkt nacheinander.

---

## JavaScript (Browser) — Minimales Live-Dashboard
...
Öffnen Sie die Datei in einem beliebigen Browser — CORS ist absichtlich weit geöffnet.

---

## Python — Alle Miner zu einer bestimmten Zeit neustarten
...
for ip in ips:
    try:
        ...
        print(f"Neustart gesendet an {ip}")
    except Exception as e:
        print(f"{ip}: {e}")
```

Vermeiden Sie es, dies häufiger als nötig auszuführen — jeder Neustart verwirft die Stratum-Sitzung und alle ausstehenden Shares.
