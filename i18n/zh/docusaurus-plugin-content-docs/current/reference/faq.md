...
可以 — 任何说 Stratum v1 的都行。已知好用的矿池见 [矿池列表](./pool-list.md)（含社区 SoloBTC 和 DGB 端点）。注意：要求高 share 难度的大型商业矿池会拒掉所有 share — 请坚持 NMMiner 官方 [SoloBTC](https://solobtc.nmminer.com) 矿池或 solo 风格端点。
...
4. **矿池难度。** 要求高 share 难度的大型商业矿池会拒掉每一个 share。请坚持 NMMiner 官方 [SoloBTC](https://solobtc.nmminer.com) 矿池。
